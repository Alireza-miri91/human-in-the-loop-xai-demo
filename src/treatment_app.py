...
RESPONSES_DIR = os.path.join(RUNTIME_DATA_DIR, 'responses', 'treatment')
...
<head>
  ...
  <style>
    ...
    .prediction-input:placeholder-shown {{
      border-color: #d1d5db;
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
      <div class="bg-white p-6 rounded-xl shadow-lg">
        <h2 class="text-xl font-semibold mb-4 border-b pb-3 text-gray-800">1. Model Constraints</h2>

        <!-- A. Direction -->
        <div class="mb-8">
          <p class="text-base text-black-500 mb-3">Tell the model how rentals should move as a feature rises: <strong>increasing</strong>, <strong>decreasing</strong>, or <strong>no constraint</strong>. This shapes the bike-sharing forecast to match your expectations. Any feature can be left as-is.</p>
          <div id="monoWrap" class="space-y-3"></div>
        </div>

        <div class="mb-4 p-3 bg-yellow-100 border-l-4 border-yellow-400 text-yellow-800 font-semibold rounded">
          <i class="fas fa-exclamation-circle mr-2"></i>
          <span>
            You can change the constraints as many times as you like! Try different combinations and click <b>Update & train Model</b> to see how the model adapts. Experiment to discover how constraints affect predictions and model behavior.
          </span>
        </div>

        <button id="updateBtn" class="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 transition-colors">
          <i class="fas fa-cog mr-2"></i>Update & train Model
        </button>
      </div>
    </div>
    ...
    <div class="lg:col-span-2">
      <div class="bg-white p-6 rounded-xl shadow-lg">
        <h2 class="text-xl font-semibold mb-4 border-b pb-3 text-gray-800">2. Model Results</h2>
          ...
          <div class="space-y-6">
            <div>
              ...
              <div id="shapePlot" class="bg-gray-50 rounded-lg p-4 min-h-96 flex items-center justify-center">
                <p class="text-gray-500">Model shape functions will be shown here after you set the constraints.</p>
              </div>
              <div class="mt-2 text-xs text-gray-500">
              </div>
            </div>
                ...
                <div id="userPredictionsSection">
                  ...
                  <div id="userPredictions" class="space-y-2 text-sm">
                    <div class="flex justify-between items-center p-2 bg-white rounded border">
                      <span class="text-gray-600">Click "Update & train Model" to see sample conditions</span>
                    </div>
                  </div>
...
/* -------- state -------- */
let constraints = { monotonicity:{} };
let currentModel = null;
let currentMetrics = null;

/* -------- helpers -------- */
function renderMono() {
  // Get numerical features dynamically from the predictors
  const numericalFeatures = predictors.map(p => p.id);
  monoWrap.innerHTML = predictors.map(p => {
    const isNumerical = numericalFeatures.includes(p.id);
    const disabledClass = isNumerical ? '' : 'opacity-50 cursor-not-allowed';
    const disabledAttr = isNumerical ? '' : 'disabled';
    return `
    <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg border">
      <span class="font-medium text-gray-700">${p.name} ${!isNumerical ? '<span class="text-xs text-gray-500">(categorical)</span>' : ''}</span>
      <div class="flex space-x-2">
        <button data-predictor="${p.id}" data-dir="positive" ${disabledAttr}
                class="mono-btn h-8 w-8 flex items-center justify-center text-gray-400 hover:bg-green-100 hover:text-green-600 rounded ${disabledClass}">
          <i class="fas fa-arrow-up"></i></button>
        <button data-predictor="${p.id}" data-dir="negative" ${disabledAttr}
                class="mono-btn h-8 w-8 flex items-center justify-center text-gray-400 hover:bg-red-100 hover:text-red-600 rounded ${disabledClass}">
          <i class="fas fa-arrow-down"></i></button>
      </div>
    </div>`;
  }).join('');
  predictors.forEach(p=>constraints.monotonicity[p.id]='none');
}
...
(function init(){
  ...
  fetch(window.location.pathname.replace(/\/[^/]*$/, '')+'/get-subject-info')
    .then(response => response.json())
    .then(data => {
      document.getElementById('subjectInfo').textContent = `Subject ID: ${data.subject_id}`;
    })
    .catch(error => {
  ...
  //   .then(data => {
  //     if(data.error) {
  //       console.error('Error loading base shape functions:', data.error);
  //       shapePlotEl.innerHTML = '<p class="text-red-500">Error loading base model shape functions</p>';
  //       return;
  //     }
  //     if(data.shape_plot) {
  //       shapePlotEl.innerHTML = `<img src="data:image/png;base64,${data.shape_plot}" class="w-full h-auto">`;
  //     }
  //   })
  //   .catch(error => {
  //     console.error('Error:', error);
...
document.body.onclick = e=>{
  const m = e.target.closest('.mono-btn');
  if(m){
     const id=m.dataset.predictor, dir=m.dataset.dir;
     constraints.monotonicity[id] = constraints.monotonicity[id]===dir?'none':dir;
     m.parentElement.querySelectorAll('.mono-btn').forEach(b=>b.classList.remove('bg-green-200','text-green-700','bg-red-200','text-red-700'));
     if(constraints.monotonicity[id]!=='none'){
       m.classList.add(dir==='positive'?'bg-green-200':'bg-red-200',
                       dir==='positive'?'text-green-700':'text-red-700');
     }
  }
};
...
document.getElementById('updateBtn').onclick = ()=>{
  ...
  // Show loading state
  document.getElementById('updateBtn').innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Training...';
  document.getElementById('updateBtn').disabled = true;
  
  fetch(window.location.pathname.replace(/\/[^/]*$/, '')+'/train',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(params)})
    .then(r=>r.json()).then(res=>{
        if(res.error){alert(res.error);return;}
        
        // Update plots
        if(res.shape_plot) {
          shapePlotEl.innerHTML = `<img src="data:image/png;base64,${res.shape_plot}" class="w-full h-auto">`;
        }
        
        // Update predictions - show user input form first
        if(res.predictions) {
          // Store predictions globally for later use
          window.currentPredictions = res.predictions;
          displayUserPredictionInputs(res.predictions);
        }
        ...
        // Reset button
        document.getElementById('updateBtn').innerHTML = '<i class="fas fa-cog mr-2"></i>Update & train Model';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (saveFeedbackBtn) {
    saveFeedbackBtn.addEventListener('click', savePredictionFeedback);
  }
  
  // Add event listener for submit predictions button
  const submitPredictionsBtn = document.getElementById('submitPredictionsBtn');
  if (submitPredictionsBtn) {
    submitPredictionsBtn.addEventListener('click', submitUserPredictions);
  }
  
  // Add event listener for reset predictions button
  const resetPredictionsBtn = document.getElementById('resetPredictionsBtn');
  if (resetPredictionsBtn) {
    resetPredictionsBtn.addEventListener('click', resetUserPredictions);
  }
});
...
function submitUserPredictions() {
  ...
  setTimeout(() => {
    ...
    document.getElementById('userPredictionsSection').classList.add('hidden');
    
    // Show model predictions section
    document.getElementById('constrainedPredictionsSection').classList.remove('hidden');
    
    // Get predictions from the current model response (stored in sessionStorage or global variable)
    // For now, we'll need to re-fetch or store the predictions
    ...
    fetch(window.location.pathname.replace(/\/[^/]*$/, '')+'/track-interaction', {
      ...
      body: JSON.stringify({
        event_type: 'user_predictions_submitted',
...
function resetUserPredictions() {
  ...
  document.getElementById('userPredictionsSection').classList.remove('hidden');
  
  // Hide model predictions section
  document.getElementById('constrainedPredictionsSection').classList.add('hidden');
  
  // Clear all user input values
  const predictionInputs = document.querySelectorAll('[id^="userPrediction_"]');
  predictionInputs.forEach(input => {
    input.value = '';
  });
...
def create_numerical_shape_plot(model, base_model=None):
    ...
    try:
        ...
        # Use a fixed y-axis range that covers the maximum possible range across all constraint combinations
        # This ensures consistent scaling for comparison across different constraint settings
        # The range is based on the maximum possible values that could occur from any combination of constraints
        y_min = -0.8  # Maximum negative range expected across all constraint combinations
        y_max = 0.8   # Maximum positive range expected across all constraint combinations
        
        # If the current data exceeds this range, extend it dynamically
        if all_y_values:
        ...
        # Ensure 0 is always included and add some padding
        y_min = min(y_min, 0)
        y_max = max(y_max, 0)
...
def train():
    ...
    boost_rate = data.get("boost_rate", 0.1)
    reg_param = data.get("reg_param", 0.01)
    
    try:
        # Extract monotonicity constraints (all features are numerical)
        monotonicity = {}
        for feat, direction in constraints.get("monotonicity", {}).items():
            if direction != "none":
                monotonicity[feat] = 1 if direction == "positive" else -1
        ...
        metrics = calculate_metrics(model, X_test, y_test_scaled)
        # Get coefficients for constrained model (all features are numerical)
        coefficients = {}
        for i, feat in enumerate(feature_names):
            idx = model.numerical_cols.index(feat)
            coefficients[feat] = float(model.linear_model.coef_[idx])
        # Only create plot after retrain (after user input)
        shape_plot = create_plots(model, X_test, y_test_scaled)
        predictions = generate_sample_predictions(model)
        ret = jsonify({
